...
Future<List<Book>> fetchProduct() async {
    ...
    var url = Uri.parse(
        'https://planetbukutes-95487a8dd763.herokuapp.com/get-buku-keranjang-id/'+idUser.toString()+'/');
                ...
                if (snapshot.data == null) {
                    ...
                    return ListView.builder(
                        ...
                        itemBuilder: (_, index) => Container(
                                ...
                                children: 
                                [
                                    Image.network("${snapshot.data![index].fields.imageM}"),
                                    Column(
                                      mainAxisAlignment: MainAxisAlignment.start,
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        ...
                                        TextButton(
                                          onPressed: () async{
                                            int idUser = request.jsonData["user_id"];
                                            int bookId = snapshot.data![index].pk;
                                            debugPrint(bookId.toString());
                                            final response = await request.postJson("https://planetbukutes-95487a8dd763.herokuapp.com/handle-cart-flutter", 
                                        ...
                                        TextButton(
                                          onPressed: () async{
                                            ...
                                            if (response['status'] == 'success') {
                                                ScaffoldMessenger.of(context)
                                                    .showSnackBar(const SnackBar(
                                                content: Text("Buku berhasil diremove"),
                                                ));
                                                Navigator.pushReplacement(
                                                    context,
                                                    MaterialPageRoute(builder: (context) => HomePage()),
