...
class _DaftarBukuDipinjamState extends State<DaftarBukuDipinjam> {
  ...
  Future<BukuKesan> fetchProduct() async {
    ...
    var response = await http.get(
      Uri.parse('http://127.0.0.1:8000/borrowed_book_list/borrowed-book-byid/$idUser'));
                      ...
                      if (true){
                        return Container(
                          ...
                          child: Column(
                            ...
                            crossAxisAlignment: CrossAxisAlignment.center, // Align the children in the center horizontally
                            children: [
                              Image.network(snapshot.data!.dipinjam[index].image),
                              ListTile(
                                title: Text(snapshot.data!.dipinjam[index].title, textAlign: TextAlign.center,),
                                subtitle: Text(snapshot.data!.dipinjam[index].author, textAlign: TextAlign.center,),
                              ),
                              ListTile(
                                title: Text('Tanggal Pengembalian: ${snapshot.data!.dipinjam[index].deadline}', textAlign: TextAlign.center,),
                              ),
                              ElevatedButton(
                                onPressed: () async{
                                  // Call the return_book API endpoint
                                  final response = await http.get(
                                    Uri.parse('http://localhost:8000/borrowed_book_list/return-book/${snapshot.data!.dipinjam[index].peminjamanId}'), // Replace with your actual URL
                                  );

                                  if (response.statusCode == 200) {
                                    // Book returned successfully, update the UI
                                      setState(() {
                                        snapshot.data!.dipinjam.removeAt(index);
                                      });
                                  } else {
                                    // Handle error case, e.g., show an error message
                                    print('Failed to return book: ${response.statusCode}');
                                  }
                                },
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              ListTile(
                                ...
                                subtitle: Text(
                                  snapshot.data!.dikembalikan[index].author, 
                                  textAlign: TextAlign.center),
                              ...
                              ListTile(
                                ...
                                subtitle: Text(
                                  snapshot.data!.dikembalikan[index].kesan,
                                  textAlign: TextAlign.center,
                                ),
                              ),
                              Padding(
                                ...
                                child: ElevatedButton(
                                  onPressed: () {
                                    Navigator.pushReplacement(
                                      context,
                                      MaterialPageRoute(builder: (context) => KesanFormPage(bookId: snapshot.data!.dikembalikan[index].bookId)),
                                    );
                                  },
